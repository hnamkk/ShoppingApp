...
class NotificationHelper {
  ...
  static Future<void> createNotification({
    ...
    // Ưu tiên userId được truyền vào, nếu không có thì lấy từ auth
  ...
  static Future<void> sendOrderCreatedNotification(
      String orderId, double totalAmount, {String? userId}) async {
  ...
  static Future<void> sendOrderStatusNotification(
      String orderId, String status, {String? userId}) async {
    String title = '';
    String message = '';
    ...
    switch (status) {
      case 'preparing':
        ...
        message = 'Đơn hàng #${orderId.substring(0, 8).toUpperCase()} đang được chuẩn bị';
        break;
      case 'delivering':
        title = 'Đang giao hàng';
        message = 'Đơn hàng #${orderId.substring(0, 8).toUpperCase()} đang trên đường giao đến bạn';
        break;
      case 'delivered':
        title = 'Đã giao hàng';
        message = 'Đơn hàng #${orderId.substring(0, 8).toUpperCase()} đã được giao thành công. Cảm ơn bạn đã mua hàng!';
        break;
      case 'cancelled':
        title = 'Đơn hàng đã hủy';
        message = 'Đơn hàng #${orderId.substring(0, 8).toUpperCase()} đã được hủy';
        break;
    }
  ...
  static Future<void> sendCartReminderNotification(int itemCount, {String? userId}) async {
    await createNotification(
  ...
  static Future<void> sendLowStockWarningNotification(
      String productName, String productId, {String? userId}) async {
    await createNotification(
  ...
  static Future<void> sendSystemNotification(String title, String message, {String? userId}) async {
    await createNotification(
      title: title,
      message: message,
      type: 'general',
